...
# Import requests at the top if not already imported
import requests

# Full AI Analysis Endpoint
@app.post("/api/analyze-video")
def analyze_video_ai(request: dict):
    try:
        video_url = request.get("video_url", "")
        title = request.get("title", "Video Analysis")
        ...
        # Call GPT-5 for real AI analysis
        headers = {
        ...
        prompt = f"""Analyze this video for viral clip potential:

Title: {title}
URL: {video_url}
Description: {description}

Return 3 viral clip suggestions with timestamps, viral scores (1-10), hooks, and platform recommendations.
Format as JSON with clips array."""
        ...
        if response.status_code == 200:
            ...
            return {
                "success": True,
                "video_url": video_url,
                "ai_analysis": {"content_analysis": ai_content},
                "source": "ai",
